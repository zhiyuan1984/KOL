...
| `stop_crawl`        | 停止采集和浏览器                                             |
| `get_creators`      | 分页读取标准化创作者。**入参**只接受 `platform`、`offset`、`limit`；发 `page` / `page_size` 会被忽略并返回全量。每条含 `platform`、`platform_creator_id`、`nickname`、`followers`、最近 10 条 `views`；响应可带 `has_more` / `total` |
