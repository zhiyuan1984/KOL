...
| 页面 | 角色 | 只回答 | 禁止 |
|---|---|---|---|
| **Home** | 现在做什么 / 等待诚实 | 平台模式：今日任务、我的待办。KOL 试点另挂 AI发现、我跟进的红人（宪法 §4.2；不是平台一等清单） | 正式 15 阶段资产板、管理端权限树、连接器治理目录 |
...
本面及同列能力面与数字员工开工入口、今日任务队列相互独立（ADR-015），不隶属 KOL Agent，也不因“有没有进行中任务”才存在。KOL / 合作只可作为数据或示例，不得定义本面 IA。治理仍只在管理端枢纽 / 详情；使用面与治理面不因本条而合并。

员工知识库 `/kb` 同属一等能力面：员工只查找、预览、收藏、用于当前任务；停用、发布、版本、范围只在管理端（ADR-021）。技能同属一等能力：侧栏是否露出是 UX，不是「员工永远不许看见」；仍禁止能力图鉴压过任务脊柱，也禁止把技能目录塞进数字员工簇或做成连接器治理入口（ADR-023）。
...
未发布 Agent 仍按宪法 §5 / 既有 FS：员工可看说明、不可提交。本页是管理端改变该状态的地方，不是员工空态的副本。不再把 `UX-AGENT-UNPUBLISHED` 当独立合同 ID。
