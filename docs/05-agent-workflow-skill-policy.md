...
Host 本地阶段码与 Starry 原生码通过 `LEGACY_STAGE_ALIASES` / `toLegacyStarryStage` 互转（ADR-011）：读入归一成 Host-local；`starryStageWriteFields` → `changeLifecycleStage` 只写 **相邻前进** 的 Starry 原生码（如 `NEGOTIATING` → `BUSINESS_NEGOTIATION`）和已知 `lastLifecycleId`。人确认的 skip kind / 原因只存在确认卡、审计和本地协作。远程不能一次落到非相邻阶段：跨格 skip 由 `planStarryAdjacentWalk` 逐格 walk，一格失败即停。
