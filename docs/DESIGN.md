...
9. **表面分三级，不靠颜色分层：** `--bg`（画布）/ `--surface`（侧栏、工具栏、输入底）/ `--surface-hover`（悬停与选中底）。禁止用整块灰底充当装饰。
10. **只标例外。** 默认态不标注；只有 L3（需确认）、异步作业、异常才打标记。**「只读」这类默认值不标**——满屏同款标签等于噪声，是页面变「脏」的主因之一。
11. **卡片元素上限：1 图标 + 1 标题 + 1 描述 + 1 动作行（≤2 个按钮）。** 来源、来源系统名、内部编号等元信息归详情列，不上卡片。
...
15. **动作按钮必须有可见边框。** 只有真正最低强调的动作（如容器内的第三顺位操作）才允许无边框；「新建会话」这类并列动作必须在视觉上有框。
