...
18. **颜色只有两个来源。** ① **中性阶**：只有下列角色，每角色**只许一个 token** —— 画布 `--bg` / 极浅面 `--ds-surface` / 悬停面 `--surface-hover` / 分隔线 `--border` / 控件边框 `--control-border` / 弱化文字 `--text-muted` / 正文 `--text`。② **主色派生**：只许 `--primary`、`--primary-hover`、`--primary-tint`、`--primary-tint-strong`（后两者由 `color-mix` 从 `--primary` 派生）。
    **禁止新增任何灰度 hex 或浅主色配方。** 现存 **4 套平行灰阶**（OpenAI-quiet / Design System v1 / 壳杂项 / 遗留紫蓝）与 **3 套浅主色配方**（`--color-pink-tint`、`--color-pink-tint-soft`、`--ds-selected-bg`）全部进 §8 迁移清单，不再用于新代码。

> **反扩散的验收判据（可执行）：** 本页渲染出的**不重复计算色值 ≤ 20 种**，且所有 SVG 的渲染宽度**只取 `--icon-sm` / `--icon-md` 两档**——见 `frontend/e2e/skills-catalog.spec.ts`。多一个灰、多一个粉、多一档图标尺寸，断言就会失败。**这才叫"有保障"：不靠人记得，靠门禁挡住。**
