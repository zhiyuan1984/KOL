...
| 位置 | 必须 | 禁止 |
|---|---|---|
| 员工侧栏 | 工作入口停在 Home / 进行中、`/agents`（文案「数字员工」，岗位专家）、Pipeline（KOL 试点资产）、知识、审批、考试、Settings。可选「连接器」入口只链员工使用面（`/connectors`），放资产簇或 Settings 邻接，不进数字员工簇。技能若露出侧栏，必须是独立一等入口，不是数字员工簇里的图鉴。确认队列在 Home（结果待确认 / 等审批），不另做「等我确认」顶栏 | 「技能目录 · **连接器**」或任何员工默认项深链 `/admin/connectors`；侧栏再挂一条克隆审批的「等我确认」；发明可见组标题；专家团入口或占位；把未实现的「数字团队」做成专家团冒名；把技能图鉴塞进数字员工簇 |
