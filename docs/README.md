...
`references/` 默认只用于研究和追溯。例外：`references/openai-style.md` 是默认观感 mood 来源（主按钮仍 MASTER `--primary`；token hex 不进 `design.md`）。ADR 只回答「为什么」，不能覆盖现行契约。
...
| **I** UI 设计 | [`design.md`](design.md) | 页面视觉法；默认观感 OpenAI-quiet；**token 仍是** [`MASTER.md`](design-system/kol-workbench/MASTER.md) |
...
| 页面视觉与布局 | I | `design.md`（入口）；token = `MASTER.md`；默认观感 OpenAI-quiet（主按钮仍 `--primary`） |
