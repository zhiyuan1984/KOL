...
20. `19-ui-ux-constitution.md`：员工端体验宪法（核心闭环、四页分工、员工侧栏 IA、并列能力面与 Agent/任务解耦、Agent 表面、结果优先）
21. `20-visual-design-system.md`：KOL-UI 工作台视觉/token 默认（Linear 密度、shadcn 基底、indigo 主色）
22. `21-admin-employee-page-roles.md`：管理端 ↔ 员工端页面角色（连接器治理枢纽/详情 vs 员工使用面、`/admin/agents` 治理、导航与遗留收敛；并列能力面独立于 Agent/任务队列见 `19`；不改 `19` 四页法律）
...
- `../experts/kol/manifest.yaml`：DigitalEmployee 发布资产（API 命名 `expert:kol`）；与 Agent 包分离。见 `evidence-expert-manifest-2026-09-14.md`。
...
- `evidence-expert-manifest-2026-09-14.md`：ExpertManifest / `expert:kol` 首版后端；召唤只建绑定会话；缺字段诚实列出。
