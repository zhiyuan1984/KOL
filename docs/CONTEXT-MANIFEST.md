...
Pipeline / Home「AI发现」「我跟进的红人」是 KOL 试点特化，不是平台一等清单。「我跟进的红人」是对象跟进面，不是第二套待办、不是 15 段主筛（ADR-030）。对象管理可表现为助理结果；员工表面唯一范式是助理优先（ADR-032）。

## UI 任务顺序

```text
B CONSTITUTION.md（§4 表面职责；L1–L3 / SEND≠STAGE 高于审美）
...
→ I design.md（视觉入口；禁止写 hex）
→ I MASTER.md（token 源；mood 冲突时 OpenAI 胜；主填充 = 产品粉红）
→ I pages/<当前页面>.md
→ I references/openai-style.md（OpenAI mood/spec；quiet/white/hairline/capsule 冲突时胜出；黑钮不是产品 CTA 法）
```

强制视觉链（headline）：**CONSTITUTION → ui-ux-pro-max → design.md → MASTER**。

先建立 B / F / G / H 事实，再进入 `ui-ux-pro-max`。它是外观 UIUX 的必经分析步，不是「仅针对未解决问题」的末端对照。OpenAI 对齐且不违反宪法的输出可以驱动 MASTER 更新；仍不能覆盖 A–K 产品不变量，也不得发明 Glassmorphism / 随机品牌字体 / 橙强调。

## 路由表

| 任务 | 默认法律层 | 条件触发后再加载 |
|---|---|---|
| Home | B §4.2（跟进面=对象管理，ADR-030；助理优先，ADR-032）；I `pages/home.md` | 发送 / 阶段 / 导入：G 硬不变量；阶段边：E `business-rules/stage-transitions.md`。不把 #120 责任桶 tablist 读成法律 |
...
| 纯视觉 / 组件 | I `CONSTITUTION → ui-ux-pro-max → design.md → MASTER` + 当前页 | token 只改 MASTER；禁止在 `design.md` 复制 hex。OpenAI mood/spec 读 `references/openai-style.md`；mood 冲突时 OpenAI 胜。~~主 CTA = 黑 / Obsidian 实底~~ **修订（ADR-031）：** 主填充 = 产品粉红（`--primary`，hex 只住 MASTER）。追溯迁移才读 `20` |
...
- `references/`：历史提示词与其它风格研究不是实施规则。例外：`references/openai-style.md` 是默认观感 mood/spec；quiet/white/hairline/capsule 冲突时 OpenAI 胜，须和解 MASTER。主填充以 MASTER `--primary` 粉红为准（token hex 仍只住 MASTER，不进 `design.md`）。
...
3. 外部建议只进入当前研究笔记；未经验证不得写入 B、G、I 的 MASTER / `design.md`。例外：视觉链中的 `ui-ux-pro-max` 是必经分析；其 OpenAI 对齐且不违反宪法的建议可以驱动 MASTER 更新（仍禁止把 hex 写入 `design.md`）。
...
- 修改全局视觉 token：改 **I** 的 `MASTER.md`，并同步实现 token。`design.md` 是设计法入口，禁止写入 hex。MASTER 与 OpenAI 在 mood 上冲突时 OpenAI 胜；主填充按产品粉红和解，不得把冲突 hex 留在 MASTER。
