...
→ I MASTER.md（token 源；与 OpenAI 审美冲突时 OpenAI 胜，须和解）
→ I pages/<当前页面>.md
→ I references/openai-style.md（OpenAI mood/spec；与 MASTER 审美冲突时胜出）
```
...
| 纯视觉 / 组件 | I `CONSTITUTION → ui-ux-pro-max → design.md → MASTER` + 当前页 | token 只改 MASTER；禁止在 `design.md` 复制 hex。OpenAI mood/spec 读 `references/openai-style.md`；与 MASTER 冲突时 OpenAI 胜，须和解 MASTER（主 CTA = 黑 / Obsidian 实底）。追溯迁移才读 `20` |
...
- `references/`：历史提示词与其它风格研究不是实施规则。例外：`references/openai-style.md` 是默认观感 mood/spec；与 MASTER 审美冲突时 OpenAI 胜，须和解 MASTER（token hex 仍只住 MASTER，不进 `design.md`）。
...
- 修改全局视觉 token：改 **I** 的 `MASTER.md`，并同步实现 token。`design.md` 是设计法入口，禁止写入 hex。MASTER 与 OpenAI 审美冲突时 OpenAI 胜，不得把冲突 hex 留在 MASTER。
