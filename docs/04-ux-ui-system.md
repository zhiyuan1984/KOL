...
员工端四页分工（Home = 现在做什么 / Pipeline = 正式生命周期资产 / Chat = 完成一件任务 / Admin = 谁与审计）以 `19-ui-ux-constitution.md` 的页面角色法律为准。Home **内**四模式（今日任务 / 我的待办 / AI发现 / 我跟进的红人）见 `19` 与 ADR-018，不是第五页。Pipeline 不得复用首页四模式、待办桶或会话技能启动器。员工侧栏只是到达这些页的 chrome：定时任务归今日工作簇，组间用分割线而不是可见的「今日 / 智能体 / 资产」标题。管理端配套套件（连接器枢纽/详情、Agent 治理、授权与绑定审计）与员工表面的硬边界见 `21-admin-employee-page-roles.md`；Admin 不是第二套 Home / Pipeline / Agents。
