...
Home AI发现跟进写入 Starry 的物理路径（产品锁定见 ADR-022）：员工确认后由 Host Gateway 调 `starrykol.importKolProfilesFromCrawler`（单行映射亦可；无 `contactEmail` 仍写，禁止编造邮箱、禁止只落本地）。采集完成只入库 CreatorCandidate，**禁止** MediaCrawler 任务结束时直接写 Starry。`upload_creators` / `KOL_INGESTION_URL` 仍是 Host 侧入库，不是 Starry 写入。粉丝数门槛只在 Host 列表预览与写入前复核；MediaCrawler 工具无 min-followers。本路径禁止 `sendEmailNow`、`changeLifecycleStage`、`decryptKolContact`。闸门：`policies/import_creator.yaml`（L3，每 `source_batch` 一次确认）。
