...
| Header | `X-MCP-API-KEY: <EMAIL_AGENT_MCP_API_KEY>` |
...
```json
{
  "mcpServers": {
    "email-agent": {
      ...
      "headers": {
        "X-MCP-API-KEY": "<EMAIL_AGENT_MCP_API_KEY>"
      }
    }
  }
}
...
| Tool | 说明 |
|------|------|
| `listDictionaryOptions` | 通用字典，如 `kol_primary_platform`、`kol_email_ai_language` |
