...
| 7 | `styles.css`、`today-plan-board.css`、`today-plan-progress.css`、`TaskBoard.tsx`、`TodayPlanProgress.tsx`、`Home.tsx`、`ScopeWorkspace.tsx`、`ResultRail.tsx`、`taskStarters.ts`、标签映射 | UI/UX 专家 / 前端专家 | in progress | 用户基于 `v 8e5c807` 与本轮截图确认：结果右栏优先、紧凑对齐；2026-09-24 移除右栏重复的“今日工作计划”可视标题/图标与“7 项任务”计数（保留无障碍标题），使搜索/操作行上移；“上一版计划”默认折叠并压为辅助小字入口；历史计划步骤逐项读取 `created_at/time/timestamp/updated_at` 并显示时间；今日技能模板及相关标签改为“今日任务”；模式入口移除计数徽标并改为自然宽度单行排列；本轮修正容器宽度 560px 以下标题与统计被拆行的问题，改为同一标题行、统计按需换行；今日/待办结果右栏移除“Host 校验后的计划结果”等冗余来源文案，并避免 current/unknown freshness 单独生成空白上下文行。代码 `35fcfa1`、`c87d367`、`d1e72cd` 已推送并部署；远端 systemd active，应用 `/api/version` 返回 `d1e72cd` | 本轮未运行 E2E；浏览器自动化曾无法连接，尚无 1280×800 新截图核对视觉与横向溢出，也未完成 DESIGN 其余宽高、键盘焦点、触摸路径矩阵 |
| 8 | ResultRail/Host DTO 字段对齐、`git diff --check`、前端类型检查、远端发布核验 | 测试经理 / UI/UX 专家 | partial | 前端 `npm run typecheck` 通过；本轮 `git diff --check` 通过；服务器完成构建并重启，systemd active，远端 `/api/version` 返回 `d1e72cd`；按用户要求未运行 E2E | 截图、焦点、滚动边界、短视口与触摸矩阵仍无证据；契约/运行测试未运行 |
