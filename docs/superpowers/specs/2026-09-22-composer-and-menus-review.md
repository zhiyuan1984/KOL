...
### 2.4 + 菜单：图 3 → 图 4（已实施；2026-09-23 按所有者澄清改为「分组平铺 + 搜索」，见 §5.6）
...
### 5.6 + 菜单改「一个面板 + 搜索 + 分组平铺」（2026-09-23）

所有者澄清图 4 的意图：**不是**两级级联，而是**分组平铺** —— 一个面板里直接看到 文件 / 知识库 /
技能 / 数字员工 等分组的真实条目，顶部一个搜索框快速查找；「不需要去二级菜单」。

- 结构：`PlusMenu` 删掉 `submenu` / `PlusSubpanel` / `Subpanel` 与悬停展开，一次渲染八组：
  添加 / 最近的文件 / 知识库 / 技能 / 数字员工 / 连接器 / 添加到项目 / 作业，每组的真实条目直接平铺。
- 顶部搜索（`[data-composer-menu-search]`）：打开即聚焦；输入即跨组过滤；命中组名时整组保留；
  无结果给「没有匹配的条目」。**回车永不提交提问框**（无查询时回车不做任何事）。
- 行：`图标 + 名称 + 一句话说明`；技能的说明位放阶段名（发现 / 画像与评估 / 跟进 SOP / 邮件与审批 / 其他），
  L3 技能用技能页同一套 `skill-mark is-write`「需确认」标记；空分组（技能还在异步加载）不渲染。
- 键盘：搜索框 ↓ 进第一行、↑ 回搜索框、Home/End 首尾、Esc 关闭并把焦点还给「+」按钮、
  任意可打印字符回到搜索框继续输入。
- 组尾的「查看全部技能 →」是**页面链接**，不是子菜单；过期的连接器不静默禁用，给出原因 +
  去连接器页的恢复入口（面板保持打开）。
- 门禁：`workbench.spec.ts:551` 的老断言（技能行要有 `.cascade-icon`）此前一直红 —— 技能行根本没有图标；
  平铺后技能行带图标，这条门禁**转绿**；`composer-prompt-input` 的菜单断言改为「无子面板、有搜索框、
  技能条目直接可见、搜索后只剩一条」。

### 5.7 分组定稿 + 去掉「添加 / 作业」组与提问框的技能触发器（2026-09-23 第三轮）

所有者定稿：**分组顺序 = 文件 / 技能 / 知识库 / 数字员工 / 连接器 / 项目**，没有「添加」和「作业」组；
「输入框的技能也不需要了」；技能**全平铺**（不做每组 N 条折叠，靠顶部搜索）。

- `PlusMenu`：`添加` 组取消，上传入口（上传文件 / 上传图片）并入 **文件** 组与最近的文件同列；
  `作业` 组与「发现红人模板」整行删除（`onOpenDiscovery` prop 一并删掉）；`添加到项目` 组改名 **项目**；
  技能组内保留阶段说明与「查看全部技能 →」页面链接。
- 提问框的 **技能触发器整块删除**（`.composer-skill-wrap` + 按钮 + 它的 SkillMenu 弹层）与
  `SkillMenu.tsx` 文件、相关 CSS 一起清掉；技能只从 + 菜单进（一个入口）。
- Discovery 模板入口：另一个会话已在 `DiscoveryPanel` 去掉模板按钮，本轮的 + 菜单又去掉「作业」组 ——
  于是模板入口在 UI 上不再存在，发现任务改为从「AI发现」面的条件卡开始。
  `home-discovery-pane.spec.ts` 的那条用例改为断言「+ 菜单不再提供发现模板」。
- 兼容：`ComposerDock` 的 `onOpenDiscoveryTemplate` 目前无人使用，签名暂时保留（Home.tsx 有并行会话在改，
  不动它的调用点），待其提交后与调用点一起删除。
- 实测（1280×720）：六组行数 6 / 48 / 2 / 3 / 4 / 2（共 65 行），无作业组、无发现模板行，
  文件组首行是「上传文件」，提问框里 `[data-composer-skill-trigger]` 与 `[data-composer-skill-menu]` 均为 0。
- 门禁：`workbench:551`、`home-discovery-pane` 的新用例、`composer-prompt-input:217`（会话提问框）全绿；
  `composer-prompt-input:72/254` 两条既有红灯未变（宽度 766、焦点环底色口径）。
