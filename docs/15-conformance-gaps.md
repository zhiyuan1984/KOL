...
当前项目是 **KOL 试点原型/过渡实现**，不是已经满足企业多公司、多组织、多品牌和配置化 Agent 愿景的平台。现行 chrome 偏 KOL（Home「AI发现 / 我跟进的红人」等）是**试点实现快照**，不是平台 IA 法律。员工侧栏挂「生命周期」/ `/pipeline` 即使仍出现在代码里，也已违宪；一等能力清单与试点特化以 `CONSTITUTION.md` §4.1–4.2 为准。主线 Markdown 现在可以作为目标架构和评审基线，但还不能单独指导生产实现。
