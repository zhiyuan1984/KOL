...
> 员工 `/approvals` 是宪法 §4.1 #7 一等能力面：确认与审批队列，不是第二套 Home。本文件约束本页布局、L3 确认与回执。视觉 token 以 `MASTER.md` 为准；`docs/references/openai-style.md` 是默认观感来源（主按钮仍 MASTER `--primary`）。
