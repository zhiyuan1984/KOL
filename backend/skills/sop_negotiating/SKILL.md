...
id: sop_negotiating
title: SOP · 商务谈判
description: 商务谈判 的版本化 SOP。只准备资料和下一步，不自动发信，不自动改正式阶段。
category: 商务
profile: negotiation
output: task_result
funnel: biz
...
对照 `backend/src/stages.ts` 的主阶段 NEGOTIATING。Codex app-server 按本 Skill 展示当前步骤、需要输入、完成证据和下一步。不发信、不改阶段。
