---
id: sop_sample_pending
title: SOP · 待寄样
description: 待寄样 的版本化 SOP。只准备资料和下一步，不自动发信，不自动改正式阶段。
category: 寄测
profile: execution
output: task_result
funnel: sample
...
sop_id: SAMPLE_PENDING
version: 2026-09-08.1

对照 `backend/src/stages.ts` 的主阶段 SAMPLE_PENDING。Codex app-server 按本 Skill 展示当前步骤、需要输入、完成证据和下一步。不发信、不改阶段。
